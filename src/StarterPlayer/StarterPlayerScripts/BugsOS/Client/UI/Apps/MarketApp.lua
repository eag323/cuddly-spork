...
local function makeButton(parent, text, x): TextButton
...
function MarketApp.Mount(target, context)
	...
	local welcome = Instance.new("TextLabel"); welcome.Size=UDim2.new(1,-12,0,22); welcome.Position=UDim2.fromOffset(6,28); welcome.BackgroundTransparency=1; welcome.TextXAlignment=Enum.TextXAlignment.Left; welcome.TextColor3=Color3.fromRGB(255,255,255); welcome.Text="Welcome to the Food Market"; welcome.Parent=root
	priceLabel = Instance.new("TextLabel"); priceLabel.Size=UDim2.new(1,-12,0,22); priceLabel.Position=UDim2.fromOffset(6,52); priceLabel.BackgroundTransparency=1; priceLabel.TextXAlignment=Enum.TextXAlignment.Left; priceLabel.TextColor3=Color3.new(1,1,1); priceLabel.Parent=root
	deltaLabel = Instance.new("TextLabel"); deltaLabel.Size=UDim2.new(1,-12,0,20); deltaLabel.Position=UDim2.fromOffset(6,74); deltaLabel.BackgroundTransparency=1; deltaLabel.TextXAlignment=Enum.TextXAlignment.Left; deltaLabel.TextColor3=Color3.fromRGB(170, 210, 255); deltaLabel.Parent=root
	chartFrame=Instance.new("Frame"); chartFrame.Size=UDim2.new(1,-12,0,160); chartFrame.Position=UDim2.fromOffset(6,98); chartFrame.BackgroundColor3=Color3.fromRGB(16,32,54); chartFrame.BorderSizePixel=0; chartFrame.Parent=root
	local b10=makeButton(root,"Sell 10%",10); local sell10Remote = context.Remotes.MarketSellFood; b10.Activated:Connect(function() sell10Remote:FireServer({SellPercent=10}) end)
	local b50=makeButton(root,"Sell 50%",190); local sell50Remote = context.Remotes.MarketSellFood; b50.Activated:Connect(function() sell50Remote:FireServer({SellPercent=50}) end)
	local all=makeButton(root,"Sell All",370); local sellAllRemote = context.Remotes.MarketSellFood; all.Activated:Connect(function() sellAllRemote:FireServer({SellPercent=100}) end)
	local autoPanel = Instance.new("Frame"); autoPanel.Size=UDim2.new(1,-12,0,110); autoPanel.Position=UDim2.fromOffset(6,270); autoPanel.BackgroundColor3=Color3.fromRGB(16,32,54); autoPanel.BorderSizePixel=0; autoPanel.Parent=root
