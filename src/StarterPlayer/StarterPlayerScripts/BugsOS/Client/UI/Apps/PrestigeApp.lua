...
function PrestigeApp.Mount(target, context)
 ...
 confirm.Activated:Connect(function() local lf=((((context.State.PlayerData or {}).Currencies or {}).LifetimeFood) or 0); if lf<PRESTIGE_REQUIREMENT then return end; if type(context.UI.ShowConfirmPopup)=="function" then context.UI.ShowConfirmPopup("Prestige and reset current run progress?", function() context.Remotes.PrestigeRequest:FireServer({Confirm=true}) end) else context.Remotes.PrestigeRequest:FireServer({Confirm=true}) end end)
end
