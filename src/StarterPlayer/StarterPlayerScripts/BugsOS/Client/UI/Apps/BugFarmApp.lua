...
local function renderFarmersTab(context, scroll, bugs, inventory, owned, farmerSlots, combatSlots)
	...
	local habitat = makeCard(scroll, UDim2.new(1,-20,0,172)); habitat.Name = "HabitatShowcaseFrame"; habitat.LayoutOrder = 1; habitat.BackgroundColor3 = Color3.fromRGB(24,55,38); habitat.ClipsDescendants=true
	local habitatStroke = habitat:FindFirstChildOfClass("UIStroke")
	if habitatStroke then habitatStroke.Color = Color3.fromRGB(90, 235, 180) habitatStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border end
	local ground = Instance.new("Frame"); ground.Size=UDim2.new(1,0,0,34); ground.Position=UDim2.new(0,0,1,-34); ground.BackgroundColor3=Color3.fromRGB(17,40,29); ground.BorderSizePixel=0; ground.Parent=habitat
	...
	if #equipped == 0 then local t=Instance.new("TextLabel"); t.Size=UDim2.fromScale(1,1); t.BackgroundTransparency=1; t.Text="Equip bugs to see them here."; t.TextSize=18; styleLabel(t,true); t.TextColor3=COLORS.muted; t.Parent=habitat
		...
		for _, e in ipairs(equipped) do
			...
			local function move() if not aura.Parent then return end local x=math.random(6,math.max(6,habitat.AbsoluteSize.X-64)); local y=math.random(6,math.max(6,habitat.AbsoluteSize.Y-70)); game:GetService("TweenService"):Create(aura,TweenInfo.new(math.random(6,12),Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),{Position=UDim2.fromOffset(x,y)}):Play() end
			move(); task.spawn(function() while aura.Parent and habitat.Parent do task.wait(math.random(5,10)); move() end end)
		end
	end
	local pills = Instance.new("ScrollingFrame"); pills.Size=UDim2.new(1,-20,0,54); pills.LayoutOrder=2; pills.BackgroundTransparency=1; pills.ScrollBarThickness=4; pills.AutomaticCanvasSize=Enum.AutomaticSize.X; pills.ScrollingDirection=Enum.ScrollingDirection.X; pills.Parent=scroll
	...
	local inst = Instance.new("TextLabel"); inst.Size=UDim2.new(1,-20,0,34); inst.LayoutOrder=3; inst.BackgroundTransparency=1; inst.TextXAlignment=Enum.TextXAlignment.Left; inst.TextWrapped=true; inst.Text="Equip bugs here for passive buffs. Catch bugs on your home screen to fill your collection."; inst.TextSize=13; styleLabel(inst,false); inst.TextColor3=COLORS.muted; inst.Parent=scroll
	...
	local eql=Instance.new("UIListLayout", equippedList); eql.Padding=UDim.new(0,6); eql.SortOrder=Enum.SortOrder.LayoutOrder
	...
	for _,e in ipairs(equipped) do local row=makeCard(equippedList, UDim2.new(1,0,0,68)); local n=Instance.new("TextLabel"); n.Size=UDim2.new(1,-240,0,24); n.Position=UDim2.fromOffset(60,8); n.BackgroundTransparency=1; n.TextXAlignment=Enum.TextXAlignment.Left; n.Text=getDisplayName(e.bug,e.cfg); n.TextColor3=getRarityColor(tostring(e.cfg.rarity or e.bug.Rarity or "Common")); styleLabel(n,true); n.Parent=row; local i=Instance.new("ImageLabel"); i.Size=UDim2.fromOffset(44,44); i.Position=UDim2.fromOffset(8,12); i.BackgroundTransparency=1; i.Image=getBugIcon(e.bug,e.cfg); i.Parent=row; local btxt={}; for _,b in ipairs(getFarmerBonuses(e.bug)) do table.insert(btxt, formatBonus(b)) end; local st=Instance.new("TextLabel"); st.Size=UDim2.new(1,-240,0,20); st.Position=UDim2.fromOffset(60,34); st.BackgroundTransparency=1; st.TextXAlignment=Enum.TextXAlignment.Left; st.Text= (#btxt>0) and table.concat(btxt," | ") or "No farmer bonus"; st.TextSize=12; styleLabel(st,false); st.TextColor3=COLORS.good; st.Parent=row; local rm=makeButton(row,"Remove",COLORS.danger,UDim2.fromOffset(90,30)); rm.Position=UDim2.new(1,-98,0.5,-15); rm.Activated:Connect(function() context.Controllers.BugFarm.UnequipFarmer(e.slotIndex) end) end
	...
	local hdr=makeCard(scroll, UDim2.new(1,-20,0,52)); hdr.LayoutOrder=7; local h=Instance.new("TextLabel"); h.Size=UDim2.new(0.5,0,1,0); h.BackgroundTransparency=1; h.TextXAlignment=Enum.TextXAlignment.Left; h.Text=string.format("Your Bugs (%d/%d)", #owned, maxOwned); h.TextSize=17; styleLabel(h,true); h.Parent=hdr
