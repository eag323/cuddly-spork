...
local combatTeamResultUnbind: (() -> ())? = nil
...
local combatActionInFlight: {[string]: boolean} = {}
...
local function renderCombatTeamTab(context, scroll, bugs, inventory, owned, farmerSlots, combatSlots)
	...
	local chips=Instance.new("Frame"); chips.BackgroundTransparency=1; chips.Size=UDim2.new(1,-20,0,66); chips.Position=UDim2.fromOffset(10,56); chips.Parent=slotCard
	local chipLayout=Instance.new("UIGridLayout", chips); chipLayout.CellPadding=UDim2.fromOffset(8,8); chipLayout.CellSize=UDim2.new(0.24,-6,0,28)
	for _,chip in ipairs({
		string.format("Power %s", formatNum(totalPower)), string.format("Avg Rank %.1f", (filled>0 and (totalRank/filled) or 0)),
		string.format("HP %s", formatNum(totalHP)), string.format("ATK %s", formatNum(totalATK)),
		string.format("DEF %s", formatNum(totalDEF)), string.format("Avg SPD %.1f", (filled>0 and (totalSPD/filled) or 0)),
		string.format("Slots %d/5", filled),
	}) do local b=makeBadge(chips, chip, Color3.fromRGB(39,79,109)); b.Size=UDim2.new(1,0,1,0) end
...
function BugFarmApp.Mount(target, context)
	...
	if context.Controllers and context.Controllers.BugFarm and type(context.Controllers.BugFarm.BindCombatTeamResult) == "function" then
		combatTeamResultUnbind = context.Controllers.BugFarm.BindCombatTeamResult(function(payload)
			if type(payload) ~= "table" then return end
			if payload.Uid ~= nil then combatActionInFlight["uid:" .. tostring(payload.Uid)] = nil end
			if payload.SlotIndex ~= nil then combatActionInFlight["slot:" .. tostring(payload.SlotIndex)] = nil end
			if detailTargetUid ~= nil and payload.Uid ~= nil and tostring(detailTargetUid) == tostring(payload.Uid) then
				refreshOpenDetailPopup(context)
			end
			if selectedTab == "Combat Team" then
				render(context)
...
function BugFarmApp.Unmount()
	...
	if combatTeamResultUnbind then combatTeamResultUnbind(); combatTeamResultUnbind = nil end
	closeDetailPopup()
	if bugdexInlineHost then
		BugdexView.Unmount()
		bugdexInlineHost:Destroy()
