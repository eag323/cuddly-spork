...
function BattleSimulator.Run(playerUnits, enemyUnits, seed, maxTurns)
 local rng=Random.new(math.floor(tonumber(seed)or os.time())); local turnsCap=math.max(1,math.floor(tonumber(maxTurns)or 40)); local units,log,events={},{},{}
 ...
 for turn=1,turnsCap do
  ...
  for _,actor in ipairs(units) do if actor.CurrentHP<=0 then continue end
   local opponents=alive(units,actor.Team=="Player" and "Enemy" or "Player"); if #opponents==0 then return {Winner=actor.Team,Turns=turn,PlayerRemaining=#alive(units,"Player"),EnemyRemaining=#alive(units,"Enemy"),Log=log,Events=events,FinalUnits=units} end
   ...
   target.CurrentHP=math.max(0,target.CurrentHP-damage); table.insert(log,string.format("%s%s hits %s for %d damage.",isCrit and "CRIT! " or "",actor.Name,target.Name,damage)); table.insert(events,{Type="Hit",Turn=turn,ActorName=actor.Name,TargetName=target.Name,Damage=damage,IsCrit=isCrit,TargetRemainingHP=target.CurrentHP,TargetMaxHP=target.Stats.HP,Defeated=target.CurrentHP<=0}); if target.CurrentHP<=0 then table.insert(log,string.format("%s is defeated.",target.Name)); table.insert(events,{Type="Defeated",Turn=turn,TargetName=target.Name,TargetRemainingHP=0,TargetMaxHP=target.Stats.HP,Defeated=true}) end
  end
  if #alive(units,"Player")==0 or #alive(units,"Enemy")==0 then local winner=#alive(units,"Player")>0 and "Player" or "Enemy"; return {Winner=winner,Turns=turn,PlayerRemaining=#alive(units,"Player"),EnemyRemaining=#alive(units,"Enemy"),Log=log,Events=events,FinalUnits=units} end
 end
 ...
 return {Winner=winner,Turns=turnsCap,PlayerRemaining=#alive(units,"Player"),EnemyRemaining=#alive(units,"Enemy"),Log=log,Events=events,FinalUnits=units}
end
